...
#
# FINDING (2026-07-10, first synthetic run): a guest panic does NOT halt
...
# never completes the emulation — see the FINDING above — so the step cap
...
if [ "$MODE" = "synthetic" ]; then
    if grep -q "verification failed" <<< "$R1$R2"; then
        ...
        echo "  (see the FINDING in this script's header — invalid input costs"
