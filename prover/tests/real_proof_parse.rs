...
fn parses_real_v0_18_proof_file() {
    let path = concat!(env!("CARGO_MANIFEST_DIR"), "/tests/data/real_proof_zisk_v0.18.0.bin");
    ...
    assert_eq!(hex(&out.public_values[..32]), EXPECTED_PROGRAM_VK, "programVK prefix");
    assert_eq!(hex(&out.public_values[32..64]), EXPECTED_COMMITMENT, "batch commitment word");
    assert!(
        out.public_values[64..288].iter().all(|b| *b == 0),
        "unused guest output words must be zero"
    );
    assert_eq!(hex(&out.public_values[288..]), EXPECTED_VADCOP_VK, "vadcop VK suffix");
}
